...
class PermissionController extends Controller
{
    ...
    public function index()
    {
        try{
            ...
            return view('settings.permission', compact('roles'))->with("currentModule", "settings");
        }catch (\Exception $e) {
            $bug = $e->getMessage();
                ...
                ->addColumn('action', function($data){
                    ...
                    if (Auth::user()->can('manage_permission')){
                        return '<div class="table-actions">
                                    <a href="'.url(app()->getLocale()  . '/permission/delete/'.$data->id).'"><i class="fa fa-trash f-16 text-red"></i></a>
                                </div>';
                    }else{
                        return '';
                    }
    ...
    public function create(Request $request)
    {
        ...
        try{
            ...
            if($permission){ 
                return redirect()->route("settings.permissions", app()->getLocale())->with('success', 'Permission created succesfully!');
            }else{
                return redirect()->route("settings.permissions", app()->getLocale())->with('error', 'Failed to create permission! Try again.');
            }
        }catch (\Exception $e) {
            $bug = $e->getMessage();
            return redirect()->back()->with('error', $bug);
    ...
    public function delete($lng, $id)
    {
        ...
        if($permission){
            ...
            return redirect()->route("settings.permissions", app()->getLocale())->with('success', 'Permission deleted!');
        }else{
            return redirect('404');
        }
    }
